<?php
	...
	*/
	...
	if($app_res['logon_way'] != 0)out(163,$app_res);//不是账号登录方式不允许使用当前操作
	...
	$markcode = isset($data_arr['markcode']) ? purge($data_arr['markcode']) : '';//机器码
	...
	$name = $res_wx['nickname'];
	$pic = $res_wx['figureurl'];
	...
	if($res_qq){
		...
		if($res_qq['ban'] > time())out(114,$res_qq['ban_notice'],$app_res);//账号被禁用
		if($app_res['mode'] == 'y'){$vip = $res_qq['vip'];}else{$vip = '999999999';}//判断当前收费模式
		$user_info = [
			'id'=>$res_qq['id'],
