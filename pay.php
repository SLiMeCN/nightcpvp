    ...
    <body>
									...
									<div class="card-body">
										<div id="inpitassembly" class="inpit">
											...
											<div class="li" checkbox>
												<?php
													$goods_res = Db::table('goods')->where('appid',$appid)->select();
		...
		<script>
			...
			$('#ali_submit').click(function() {
				...
				if(mobile_flag){
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&ua=1&order=";?>'+order+'<?php echo "&user={$user}&way=ali&gid=";?>'+g_id;
				}else{
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&order=";?>'+order+'<?php echo "&user={$user}&way=ali&gid=";?>'+g_id;
				}
			...
			$('#wx_submit').click(function() {
				...
				var mobile_flag = isMobile();
				if(mobile_flag){
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&ua=1&order=";?>'+order+'<?php echo "&user={$user}&way=wx&gid=";?>'+g_id;
				}else{
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&order=";?>'+order+'<?php echo "&user={$user}&way=wx&gid=";?>'+g_id;
				}
				return false;//重要语句：如果是像a链接那种有href属性注册的点击事件，可以阻止它跳转。
			});
			...
			$('#qq_submit').click(function() {
				...
				var mobile_flag = isMobile();
				if(mobile_flag){
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&ua=1&order=";?>'+order+'<?php echo "&user={$user}&way=qq&gid=";?>'+g_id;
				}else{
					window.location.href='<?php echo WEB_URL."/api.php?app={$appid}&act=pay&order=";?>'+order+'<?php echo "&user={$user}&way=qq&gid=";?>'+g_id;
				}
				return false;//重要语句：如果是像a链接那种有href属性注册的点击事件，可以阻止它跳转。
			});
