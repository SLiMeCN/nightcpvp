                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-centered table-striped dt-responsive nowrap w-100">
                                                ...
                                                <tbody>
													<?php
														$goods = Db::table('goods_order','as O')->field('O.*,U.user,U.email,G.appid,G.type,A.name as appname')->JOIN("goods","as G",'O.gid=G.id')->JOIN('app','as A','G.appid=A.id')->JOIN("user",'as U','O.Uid=U.id');
														if($so){
															$goods = $goods->where('A.name','like',"%{$so}%")->whereOr('O.order','like',"%{$so}%")->whereOr('U.user','like',"%{$so}%")->whereOr('U.email','like',"%{$so}%")->whereOr('O.name','like',"%{$so}%")->order('id desc');
														}else{
															if($see >= 0 && $appid > 0){
                                                    ...
                                                    <tr>
														...
														<td>
															<center><?php echo (empty($rows['user'])) ? $rows['email'] : $rows['user'];?></center>
                                                        </td>
														<td>
