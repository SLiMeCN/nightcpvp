                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-centered table-striped dt-responsive nowrap w-100">
                                                ...
                                                <tbody>
													<?php
														$user = Db::table('user','as U')->field('U.id,U.pic,U.email,U.user,U.name as uname,U.vip,U.reg_time,U.ban,A.name as appname,IFNULL(L.zx,0) as zai')->JOIN('app','as A','U.appid=A.id')->JOIN("(SELECT uid,COUNT(*) AS zx FROM `{$DP}user_logon` where `last_t` > {$UTT} GROUP BY uid) AS L",'U.id=L.uid');
														if($so != ''){
															$user = $user->where('A.name','like',"%{$so}%")->whereOr('U.id','like',"%{$so}%")->whereOr('U.appid','like',"%{$so}%")->whereOr('U.user','like',"%{$so}%")->whereOr('U.email','like',"%{$so}%")->whereOr('U.name','like',"%{$so}%")->order('id desc');
														}else{
                                                    ...
                                                    <tr>
														...
														<td class="media">
															...
															<div class="media-body">
																<a href="user_edit.php?id=<?php echo $rows['id']; ?>" class="text-title"><h5 class="mt-0 mb-1"><?php echo (empty($rows['user'])) ? $rows['email'] : $rows['user'];?></h5></a>
